...
L'utilisation de R confère aux données du PMSI la liberté proposée par le logiciel :
   ...
   - le reporting de l'activité en excel, pdf, word, html, ou en créant des applications ([shiny](http://shiny.rstudio.com))
...
L'engouement autour de R est lié au développement de packages intuitifs et performants : [*readr*](http://readr.tidyverse.org), [*dplyr*](https://github.com/hadley/dplyr), [*tidyr*](http://tidyr.tidyverse.org), [*magrittr*](https://github.com/tidyverse/magrittr), pour n'en citer que quelques-uns. [*pmeasyr*](https://github.com/IM-APHP/pmeasyr) s'appuie sur ces packages pour proposer des imports de données rapides sur des fichiers de taille importante (l'entité juridique de l'AP-HP est prise en charge sans problème avec un ordinateur récent).

Dans le cas de [*pmeasyr*](https://github.com/IM-APHP/pmeasyr), l'import de 100 000 rsa (partie fixe, parsing des passages unités médicales, des diagnostics associés et des actes) nécessite en moyenne 5 secondes avec un processeur i7 -- 16Go de ram.
